...
module.exports = (db) => {
  ...
  const getMenu = function(userId) {
    return db.query(`SELECT id, meal_name, description, image FROM menu
      WHERE user.id = $1
      GROUP BY id, orders.id;`, [userId])
  };

  // psuedo code guess at DB query for single item complete details
  const getMeal = function(mealId) {
    return db.query(`SELECT id, meal_name, description, image, recipe?, cooking_directions?  FROM menus
    WHERE id = $1;`, [mealId])
  };
  ...
  router.post('/signup', (req, res) => {
    ...
    const insertArray = [first_name, last_name, email, hashedPassword, cellphone_number];
    if (!first_name || !last_name || !email || !req.body.password_input || !cellphone_number) {
      res.send('Error 400: All fields are mandatory. Please complete the form and resubmit.');
    };
    db.query(`SELECT email FROM customers WHERE email = $1;`, [email])
    .then(data => {
      if (data.rows.length === 0) {
        // console.log('no data.rows');
        db.query(`INSERT INTO users (first_name, last_name, email, password, cellphone_number) VALUES ($1, $2, $3, $4, $5) RETURNING *;`, insertArray)
          .then(data => {
